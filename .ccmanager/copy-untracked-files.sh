...
# Get list of untracked directories (excluding node_modules at any level)
UNTRACKED_DIRS=$(git ls-files --others --directory | grep -v 'node_modules/')

# Get list of all untracked files (excluding node_modules at any level)
UNTRACKED_FILES=$(git ls-files --others | grep -v 'node_modules/')
...
if [ -n "$UNTRACKED_DIRS" ]; then
    ...
    while IFS= read -r dir; do
        if [ -n "$dir" ]; then
            ...
            # Copy the directory maintaining its path structure
            if [ -d "$dir" ]; then
                # Use rsync for better directory copying or tar
                # This preserves the directory structure correctly
                tar -cf - "$dir" | (cd "$DEST_DIR" && tar -xf -)
            fi
        fi
    done <<< "$UNTRACKED_DIRS"
fi
...
echo "Done! Untracked files and directories (excluding node_modules) have been copied to '$DEST_DIR'"